...
void loop() {
  while(1){
    ...
    int vctrl_mode = 115;//即串口接收为s时进入速度控制模式
    int pctrl_mode = 112;//串口输入为p时进入位置控制模式
    int interctrl_mode = 105;//串口输入为i时进入互控模式
    int monitor_mode = 109;//串口输入为m时可monitor
    Serial.println("choose mode:s/p/i/");
