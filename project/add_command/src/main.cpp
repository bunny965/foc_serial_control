...
在串口窗口中输入：M+速度，就可以使得两个电机闭环转动
比如让两个电机都以 10rad/s 的速度转动，则输入：M10
...
PIDController pid  = PIDController(P,I,D,ramp,limit);
Commander command = Commander(Serial);
void onMotion(char* cmd) {command.scalar(&target_velocity, cmd); }
void onPid(char* cmdd){ command.pid(&pid,cmdd); }
void setup() {
