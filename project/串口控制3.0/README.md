...
采用置0式使再次进入位置控制时当前位置即0位置：或可在**BLDCMotor.cpp**内的**move()**中，```shaft_angle=shaftAngle();``` 前添加标志判断是否为切换模式的第一次进入move()，如果是不获取底层sensor得到的位置,直接将**shaft_angle**、**target**、**shaft_angle_sp**设为0，但直接操作shaftAngle可能会使其丢失从开机到目前的旋转角度（或许可以从sensor再次读取）
